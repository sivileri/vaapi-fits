...
class BaseEncoderTest(slash.Test, BaseFormatMapper):
  ...
  def gen_output_opts(self):
    ...
    if vars(self).get("slices", None) is not None:
      opts += " -slices {slices}"
    if (vars(self).get("tile_cols", None) is not None) and (vars(self).get("tile_rows", None) is not None):
      opts += " -tiles {tile_cols}x{tile_rows}"
  ...
  def check_bitrate(self):
    ...
    if "cbr" == self.rcmode:
      ...
      # acceptable bitrate within 20% of bitrate
      if (os.environ.get('D3D12_VAAPIFITS_IGNORE_BITRATE_GAP') == None):
        assert(bitrate_gap <= 0.20)
